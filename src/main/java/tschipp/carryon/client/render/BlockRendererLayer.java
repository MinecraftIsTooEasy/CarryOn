...
public class BlockRendererLayer {
    ...
    public static void renderFirstPerson(EntityLiving entity, ItemStack stack, float partialTicks)
    {
        if (stack == null || stack.getItem() != CarryOnItems.TILE_ITEM) return;
     ...
     * {@code bfr.renderBlockByRenderType} path via a minimal
    ...
    {
        Minecraft mc = Minecraft.w();
        if (mc == null || mc.f == null || mc.h == null) return;
        ...
        World world = mc.f;
        int bx = ((int) mc.h.posX & ~15);
        int by = 255;
        int bz = ((int) mc.h.posZ & ~15);
        ...
        IBlockAccess brightWorld = new BrightBlockAccess(world).delegate;
        bfr rb = new bfr(brightWorld);

        bfq t = bfq.a;
        t.b();
        t.c(0xF000F0);
        t.b(-bx - 0.5, -by, -bz - 0.5);
    ...
    /**
         ...
         */
    private static class BrightBlockAccess implements IBlockAccess {

        private final IBlockAccess delegate;

        private BrightBlockAccess(IBlockAccess delegate) {
            this.delegate = delegate;
        }

        @Override
        public int h(int x, int y, int z, int min) {
            return 0xF000F0;
        }
        ...
        @Override
        public float i(int x, int y, int z, int face) {
            return 1.0f;
        }
        ...
        @Override
        public Vec3DPool getWorldVec3Pool() {
    ...
    private static void renderChestWithMeta(Block block, int meta, float brightness)
    {
        ...
        new bfr().a(block, meta, brightness);
    }
