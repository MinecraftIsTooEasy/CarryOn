...
public class PickupHandler {
    ...
    public static boolean isBlackListedBlocks(Block block)
    {
        ...
        if (block instanceof BlockEnderPortal) return true;
        if (block instanceof BlockEnderPortalFrame) return true;

        if (block instanceof BlockStrongbox) return true;

        if (block instanceof BlockCommand) return true;
    ...
    public static boolean isWhitelistedBlocks(Block block)
    {
        ...
        if (block instanceof BlockPiston) return true;

        if (block instanceof BlockRedstoneLamp) return true;
